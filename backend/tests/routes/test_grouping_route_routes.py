...
class TestGroupingRouteRoutes:
    ...
    def test_get_grouping_route_retorna_200(self, mock_service, mock_auth, client, app):
    ...
    @patch("app.middlewares.auth_middleware.user_repository")
    @patch("app.controllers.grouping_route_controller.grouping_route_service")
    def test_get_grouping_route_retorna_estrutura_correta(self, mock_service, mock_auth, client, app):
        mock_auth.find_by_id.return_value = MagicMock(id=1)
        mock_service.build_route.return_value = ROUTE_RESULT
        response = client.post("/api/grouping-routes/", json=VALID_BODY, headers=_auth_headers(app))
        ...
        assert "groupings_without_coords" in data

    @patch("app.middlewares.auth_middleware.user_repository")
    @patch("app.controllers.grouping_route_controller.grouping_route_service")
    def test_get_grouping_route_repassa_parametros(self, mock_service, mock_auth, client, app):
        mock_auth.find_by_id.return_value = MagicMock(id=1)
        mock_service.build_route.return_value = ROUTE_RESULT
    ...
    @patch("app.middlewares.auth_middleware.user_repository")
    def test_get_grouping_route_sem_latitude_retorna_400(self, mock_auth, client, app):
        mock_auth.find_by_id.return_value = MagicMock(id=1)
    ...
    @patch("app.middlewares.auth_middleware.user_repository")
    def test_get_grouping_route_grouping_ids_vazio_retorna_400(self, mock_auth, client, app):
        mock_auth.find_by_id.return_value = MagicMock(id=1)
        response = client.post(
            "/api/grouping-routes/",
    ...
    @patch("app.middlewares.auth_middleware.user_repository")
    @patch("app.controllers.grouping_route_controller.grouping_route_service")
    def test_get_grouping_route_sem_coords_validas_retorna_400(self, mock_service, mock_auth, client, app):
        mock_auth.find_by_id.return_value = MagicMock(id=1)
    ...
    def test_get_grouping_route_sem_token_retorna_401(self, client):
        response = client.post("/api/grouping-routes/", json=VALID_BODY)
        assert response.status_code == 401

    def test_get_grouping_route_metodo_get_nao_permitido(self, client, app):
        response = client.get("/api/grouping-routes/", headers=_auth_headers(app))
